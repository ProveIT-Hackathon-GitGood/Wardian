...
// ─── Vital sign options (matches backend PatientVital model columns) ───
...
export default function AnalyticsPage() {
  ...
  // All patients (assigned + unassigned are already merged in `patients`)
  ...
  // ─── Fetch vitals from backend when selected patient changes ───
  const fetchVitals = useCallback(async () => {
    if (!currentPatient?.backendId) {
      setVitalsData([]);
      return;
    }
    ...
    try {
      const data = await apiGet<BackendVitalRecord[]>(
        `/api/v1/patient-vital/patient/${currentPatient.backendId}`,
        true
      );
      ...
      setError(e?.message || 'Failed to load vitals data');
      setVitalsData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  // ─── Build chart data for a specific vital key ───
  ...
  // ─── Available vitals (not yet selected) ───
  ...
  const criticalCount = allPatients.filter((p) => p.sepsisRiskScore >= 70).length;
  const warningCount = allPatients.filter(
    (p) => p.sepsisRiskScore >= 40 && p.sepsisRiskScore < 70
  ).length;
  const avgRiskScore =
    totalPatients > 0
      ? Math.round(allPatients.reduce((acc, p) => acc + (p.sepsisRiskScore ?? 0), 0) / totalPatients)
      : 0;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4 lg:p-6 space-y-6">
        ...
        {/* Controls Card ─ Patient & Vitals selectors */}
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              ...
              >
                <SelectTrigger id="patient-selector" className="w-full sm:w-[320px]">
              ...
              <div className="p-3 bg-muted/50 rounded-lg flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <div className="text-sm">
                  ...
                  <span className="text-muted-foreground">
                    {' '}
                    — {currentPatient.age}{currentPatient.gender === 'M' ? 'M' : 'F'},{' '}
                    {currentPatient.diagnosis || 'No diagnosis'}
                  </span>
                  <span className="ml-2 inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">
                    Risk: {currentPatient.sepsisRiskScore ?? 0}%
                  </span>
                </div>
              </div>
            )}
        ...
        {/* Vitals Charts Grid */}
        {!isLoading && currentPatient && selectedVitals.length > 0 && (
              ...
              <LineChart data={data}>
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div
                        className="rounded-lg border border-border bg-card px-3 py-2 shadow-lg text-xs"
                      >
          ...
          <div>
            ...
            {trend && (
              <div className="flex items-center gap-1 mt-2 text-xs">
                ...
                <span className={trend.isUp ? 'text-critical' : 'text-success'}>
                  {trend.value}
                </span>
