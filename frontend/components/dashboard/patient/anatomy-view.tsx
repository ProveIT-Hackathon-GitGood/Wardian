...
export const AnatomyView: React.FC<AnatomyViewProps> = ({ riskScore, className }) => {
  ...
  return (
    <div className={cn("relative flex flex-col items-center justify-center p-3 bg-slate-950/40 rounded-xl border border-white/5 overflow-hidden min-h-[240px]", className)}>
      {/* Background Ambience */}
      <motion.div 
      ...
      {/* Header Info */}
      <div className="absolute top-2.5 left-2.5 z-10 space-y-0.5">
        <h3 className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Vascular Analysis</h3>
        <div className="flex items-center gap-1">
          ...
          <span className="text-[8px] font-medium text-slate-200">{status.label} Flow</span>
        </div>
      </div>

      {/* The Anatomy SVG */}
      <div className="relative w-full max-w-[140px] aspect-[1/2] z-10 mt-5">
        <svg viewBox="0 0 100 200" className="w-full h-full filter drop-shadow-sm">
      ...
      <div className="absolute bottom-2.5 right-2.5 z-10 flex flex-col items-end gap-1">
        <div className="flex items-center gap-1 bg-slate-900/80 backdrop-blur-md px-1.5 py-0.5 rounded-md border border-white/10 shadow-xl">
           <Zap className={cn("w-2 h-2", riskScore >= 70 ? "text-red-500" : "text-amber-500")} />
           <span className="text-[10px] font-black italic text-white">{riskScore}%</span>
        </div>
      </div>

      {/* Decorative Grid Lines */}
      <div className="absolute inset-0 pointer-events-none opacity-20 z-[1]">
        <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(circle, #ffffff 1px, transparent 1px)', backgroundSize: '10px 10px' }} />
      </div>
    </div>
  );
};
